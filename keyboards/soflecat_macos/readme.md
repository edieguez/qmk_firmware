...
This board uses a Pro Micro-footprint RP2040 clone controller, not the AVR Pro Micro the base `keyboard.json` targets; the `default` keymap's `rules.mk` sets `CONVERT_TO = rp2040_ce` so builds target it automatically.
